...
test_that("read_tabde works for csvy files", {
  ...
  expect_identical(
    tolower(as_sql(tres, "test.table")),
    tolower("CREATE TABLE test.table (Sepal.Length DOUBLE NOT NULL, Sepal.Width DOUBLE, Petal.Length DOUBLE, Petal.Width DOUBLE, Species VARCHAR(255), CONSTRAINT X_SPECIES_PK PRIMARY KEY (Species))")
  )
})
